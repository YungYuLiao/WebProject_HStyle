...
* 訂單詳情利用Boostrap的Collapse點按後即可展示配送詳情及訂單商品詳情，並且可檢視檢貨單和列印檢貨單，利用JavaScript的print實作列印功能
...
9. 利用HttpContext.User 屬性`this.User.Identity.Name;`，以取得已驗證的操作員工的帳號，並記錄在訂單操作(Log)中
10. 訂單狀態的下拉式選單設計上，更改為下一個進程後就不能往回選擇，避免操作者誤改
### 資料表設計:
